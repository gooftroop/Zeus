...
def import_loop(cls, instance_or_dict, field_converter, context=None,
    ...
    if strict:
        for field in set(instance_or_dict) - set(all_fields):
    ...
    for field_name, field in iteritems(cls._fields):
        ...
        if raw_value is None:
            if field_name in data:
                continue
            # bwebb - For now, do not apply defaults.
            # raw_value = field.default

        try:

            # bwebb - If the field was found in the input instance_or_dict, it is required,
            # and partials are not allowed, report an error if raw_data is falsey
            if field.required and not partial and found and raw_value == "":
                errors[serialized_field_name] = [field.messages['required']]
            elif raw_value is None:
                # bwebb - if the raw_value is non and the field is not required, just
