...
class IPv4Type(BaseType):
    ...
    def validate(self, value):
        ...
        """
        if value and not IPv4Type.valid_ip(value):
...
class StringType(BaseType):
    ...
    def __init__(self, regex=None, max_length=None, min_length=None, **kwargs):
        if isinstance(regex, list):
            self.regex = [re.compile(member) for member in regex]
        else:
    ...
    def validate_regex(self, value):
        if self.regex is not None and value:
            if isinstance(self.regex, list):
                for member in self.regex:
                    if self.member.match(value) is None:
                        raise ValidationError(self.messages['regex'])
            elif self.regex.match(value) is None:
                raise ValidationError(self.messages['regex'])
...
class URLType(StringType):
    ...
    def validate_url(self, value):
        if value and not URLType.URL_REGEX.match(value):
...
class EmailType(StringType):
    ...
    def validate_email(self, value):
        if value and not EmailType.EMAIL_REGEX.match(value):
