...
class HTTPServerRequest(object):
    ...
    def _parse_body(self):
        ...
        for k, v in self.body_arguments.items():
            self.arguments.setdefault(k, []).extend(v) if isinstance(v, list) else self.arguments.setdefault(k, v)
...
def parse_request_start_line(line):
    ...
    try:
        """
        bwebb - 08/27/15. The request comes from the iostream through _parse_header already decoded.
        Any change to alter this behavior would be too significant and deep, which would most likely
        introduce unstability and break tornado. As such, we will convert the previous niave parser
        into something more robust that is able to handle spaces inside the path by specifically
        looking for the METHOD at the first token in the line and the version at the last token in
        the line. Anything remaining will be assumed as the path.
        """
